...
async function main(){
  ...
  const fill=need>0 ? pickN(posts.filter(p=>!articles.some(a=>a.id===p.id)&&!pickedOdd.some(o=>o.id===p.id)), need, rng) : [];

  const featured={ generatedAt:new Date().toISOString(), seed:isoWeekSeed(), articles, oddities:[...pickedOdd, ...fill] };
